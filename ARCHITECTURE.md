...
| `test/` | **821 test** (76 dosya); klasör yapısı `lib/`'i aynalar |
...
| 29 | **Hukuki metinler (KVKK)** | `Legal/` (+`LegalConsentRules` · `LegalConsentWriter` · `LegalProjection` · `LegalSettings`) | `legal/documents`, `legal/documents/{type}` *(ikisi de **anonim**)* · `users/me/consents` (GET+POST) · `auth/register` gövdesindeki `consents` | `LegalAdmin` | `legal` | *(12.17'de gelecek)* | — |
...
Koda bakarak anlaşılmayan, bozulunca **sessizce** hasar veren bağımlılıklar (**74 madde**). Hepsi
